...
//        setAlphaSliderVisible(true);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int desiredWidth = parentWidth; // Force full width
        ...
        int width = desiredWidth; // Ensure full width
        int height;

        if (!mShowAlphaPanel) {
            height = (int) ((desiredWidth - PANEL_SPACING - HUE_PANEL_WIDTH) * 0.75f); // Adjust height based on width

            // Adjust height if it exceeds allowed height or in landscape mode
            if (height > heightAllowed || "landscape".equals(getTag())) {
                height = heightAllowed;
            }
        } else {
            height = (int) ((desiredWidth - HUE_PANEL_WIDTH) * 0.75f + ALPHA_PANEL_HEIGHT);
...
//		float right = left + panelSide;
        float right = dRect.right - PANEL_SPACING - HUE_PANEL_WIDTH - BORDER_WIDTH_PX * 2;
