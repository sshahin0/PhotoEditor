package com.limsphere.pe.shape;
...
public class ViewWaist extends FrameLayout implements OnTouchListener {
    ...
    class onTouch implements OnTouchListener {
        ...
        public boolean onTouch(View v, MotionEvent e) {
            if (e.getAction() == 0) {
                this.centerX = ((float) (ViewWaist.this.getLeft() + ViewWaist.this.getRight())) / 2.0f;
                this.centerY = ((float) (ViewWaist.this.getTop() + ViewWaist.this.getBottom())) / 2.0f;
                X = this.centerX;
                Y = this.centerY;
                this.startX = (e.getRawX() - ViewWaist.this.handleView.getX()) + this.centerX;
                this.startY = (e.getRawY() - ViewWaist.this.handleView.getY()) + this.centerY;
                ...
                if (newScale >= ViewWaist.this.minimumScale && newScale <= ViewWaist.this.maximumScale) {
                    ViewWaist.this.computeRenderOffset(ViewWaist.this, 0.0f, 0.0f);
                    float newRotation = (newA - this.startA) + this.startRotation;
                    ViewWaist.this.setScaleX(newScale);
                    ViewWaist.this.setScaleY(newScale);
                    ViewWaist.this.handleView.setScaleX(1.0f / newScale);
                    ViewWaist.this.handleView.setScaleY(1.0f / newScale);
                    LayoutParams params = (LayoutParams) ViewWaist.this.handleView.getLayoutParams();
                    params.setMargins(0, 0, (int) (30.0f / newScale), (int) (30.0f / newScale));
                    ViewWaist.this.handleView.setLayoutParams(params);
                }
    ...
    private class ScaleGestureListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public boolean onScale(View view, ScaleGestureDetector detector) {
            ...
            info.deltaScale = ViewWaist.this.isScaleEnabled ? detector.getScaleFactor() : 1.0f;
            if (ViewWaist.this.isRotateEnabled) {
                angle = Vector2D.getAngle(this.mPrevSpanVector, detector.getCurrentSpanVector());
            } else {
                angle = 0.0f;
            }
            info.deltaAngle = angle;
            if (ViewWaist.this.isTranslateEnabled) {
            ...
            info.minimumScale = ViewWaist.this.minimumScale;
            info.maximumScale = ViewWaist.this.maximumScale;
            ViewWaist.this.move(view, info);
            return false;
        }
    }
    ...
    public ViewWaist(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ViewWaist(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    @SuppressLint("NewApi") public ViewWaist(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
